...
def armor_metal_wire_and_strip(d_cl,full_info):
    ...
    if full_info['sheath_armour_info']['armour']=='双钢带铠装':
        ...
        if d_cl[0]<=10:
            ...
            return [x+d_Round_steel_wire[0]*4 for x in d_cl]
        if 10<d_cl[0]<=15:
            print("圆钢丝铠装厚度为：", d_Round_steel_wire[1],"mm")
            return [x+d_Round_steel_wire[1]*4 for x in d_cl]
        if 15<d_cl[0]<=25:
            print("圆钢丝铠装厚度为：", d_Round_steel_wire[2],"mm")
            return [x+d_Round_steel_wire[2]*4 for x in d_cl]
        if 25<d_cl[0]<=35:
            print("圆钢丝铠装厚度为：", d_Round_steel_wire[3],"mm")
            return [x+d_Round_steel_wire[3]*4 for x in d_cl]
        if 35<d_cl[0]<=60:
            print("圆钢丝铠装厚度为：", d_Round_steel_wire[4],"mm")
            return [x+d_Round_steel_wire[4]*4 for x in d_cl]
        if 60<d_cl[0]:
            if full_info['sheath_armour_info']['armour']!='粗圆钢丝铠装':
                print("圆钢丝铠装厚度为：", d_Round_steel_wire[5],"mm")
                return [x+d_Round_steel_wire[5]*4 for x in d_cl]
            else:
                print("粗圆钢丝铠装厚度为：", d_Round_steel_wire[6],"mm")
                return [x+d_Round_steel_wire[6]*4 for x in d_cl]
